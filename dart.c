...
int main (int argc, char *argv[]) {
  ...
  if (strcmp(&fieldvalue[command_param][0],"objectinsertion")==0) {
   ...
   if (strcmp(&fieldvalue[objtype_param][0],"quicktimemovie")==0) {
    ...
    sprintf(new_object_num,"%d",rannum);
    ...
    sprintf((char *)&values,"/usr/bin/transform  %s %s %s &",new_object_name,&fieldvalue[objtype_param][0],new_object_num);
    shresult=system( values );
